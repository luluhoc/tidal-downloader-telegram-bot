    ...
    this.api.interceptors.request.use((config) => {
      if (settingsManager.token.accessToken) {
        config.headers['Authorization'] =
          `Bearer ${settingsManager.token.accessToken}`;
      }
      return config;
    });
  ...
  public async checkDeviceCode(deviceCode: string) {
    try {
      ...
      if (response.data.access_token) {
        ...
        settingsManager.token.expiresAfter =
  ...
  public async getStreamUrl(id: number): Promise<StreamUrl> {
    ...
    if (manifestMimeType.includes('vnd.tidal.bt')) {
      const manifest = JSON.parse(
        Buffer.from(response.data.manifest, 'base64').toString('utf-8'),
      );
      url = manifest.urls[0];
      urls = manifest.urls;
      ...
      const xml = Buffer.from(response.data.manifest, 'base64').toString(
        'utf-8',
      );
